...
ifeq "$(wildcard $(KVM_PLUGIN_DIR))" ""
	...
	sudo sh -c "uname -m | xargs -i jq '.configuration.arch = \"{}\"' $(KVM_PLUGIN_CONFFILE) > /tmp/kvm_plugin.tmp && mv /tmp/kvm_plugin.tmp $(KVM_PLUGIN_CONFFILE)"
	sudo sh -c "uname -m | xargs -i jq '.configuration.emulator = \"/usr/bin/qemu-system-{}\"' $(KVM_PLUGIN_CONFFILE) > /tmp/kvm_plugin.tmp && mv /tmp/kvm_plugin.tmp $(KVM_PLUGIN_CONFFILE)"
